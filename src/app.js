...
function generateIdentityToken(uuid, name, sessionID, scope, entitlements = ['game.base']) {
  const now = Math.floor(Date.now() / 1000);
  const exp = now + 3600; // 1 hour (as default in build 7)
...
function routeRequest(req, res, url, body, headers) {
  ...
  let entitlements = body.entitlements || ['game.base'];
  let sessionID = body.session || crypto.randomUUID();
  let scope = body.scope || ['hytale:game'];
  ...
  if (headers && headers.authorization) {
    try {
      ...
      if (parts.length >= 2) {
        ...
        if (payload.jti) sessionID = payload.jit;
      }
    } catch (e) {}
  }
  ...
  if (urlPath === '/game-session/child' || urlPath.includes('/game-session/child')) {
    handleGameSessionChild(req, res, body, uuid, name, sessionID, entitlements);
    return;
  }
  ...
  const accessToken = generateIdentityToken(uuid, name, sessionID, scope, entitlements);
  sendJson(res, 200, {
    success: true,
    identityToken: accessToken,
    sessionToken: generateSessionToken(uuid, sessionID),
...
function handleTokenExchange(req, res, body, uuid, name, headers) {
  ...
  const refreshToken = generateSessionToken(uuid, sessionID);
  const expiresAt = new Date(Date.now() + 36000 * 1000).toISOString();
...
function handleGameSessionNew(req, res, body, uuid, name) {
  ...
  const identityToken = generateIdentityToken(uuid, name, sessionID, scope, entitlements);
  const sessionToken = generateSessionToken(uuid, sessionID);
  const expiresAt = new Date(Date.now() + 36000 * 1000).toISOString();
...
function handleGameSessionChild(req, res, body, uuid, name, sessionID, entitlements = ['game.base']) {
  //console.log('game-session/child:', uuid, sessionID, name, entitlements);
  console.log('game-session/child:', JSON.stringify(body));

  const scopes = body.scopes || ['hytale:server'];
  const scopeString = Array.isArray(scopes) ? scopes.join(' ') : scopes;

  const entitlementsArray = JSON.parse(entitlements);

  const childIdentityToken = generateIdentityToken(uuid, name, sessionID, scopeString, entitlementsArray)
  const sessionToken = generateSessionToken(uuid, sessionID);
...
function handleSession(req, res, body, uuid, name) {
  ...
  sendJson(res, 200, {
    ...
    identity_token: generateIdentityToken(uuid, name, sessionID, scope, entitlements),
...
function handleToken(req, res, body, uuid, name) {
  sendJson(res, 200, {
    access_token: generateIdentityToken(uuid, name, sessionID, scope, entitlements),
    identity_token: generateIdentityToken(uuid, name, sessionID, scope, entitlements),
    session_token: generateSessionToken(uuid, sessionID),
    token_type: 'Bearer',
    expires_in: 86400,
    refresh_token: generateSessionToken(uuid, sessionID)
  });
}
